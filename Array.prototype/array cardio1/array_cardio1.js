...
//2.1 Crée objet {Prénom Nom : année de décès};
let yearDeath = inventors.map(inventor => {
    let nObj = {};
    nObj[`${inventor.first}  ${inventor.last}`] = (inventor.passed - inventor.year);
    return nObj;
});
...
/*let orderer = inventors.sort(function(a, b) {
...
/*
let lives = inventors.sort(function(a, b) {
...
/*
let blvParis = ["Boulevards of Paris", "City walls of Paris", "Thiers wall", "Wall of Charles V", "Wall of Philip II Augustus",
    ...
    "Boulevard Lefebvre", "Boulevard de la Madeleine", "Boulevard de Magenta", "Boulevard Marguerite-de-Rochechouart",
...
const deInBlv = blvParis.filter(blv => blv.indexOf('de') != -1);

console.log(deInBlv)
*/
/* ou la version pour console utilisateur du site
...
// 7.1 Sort the people alphabetically by first name

let alpha = people.sort((last, first) => {
     let [aLast, aFirst] = last.split(', ')
     let [bLast, bFirst] = first.split(', ');
     if (aFirst > bFirst) {
         return 1;
     } else {
         return -1
